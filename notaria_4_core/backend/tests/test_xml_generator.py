...
def test_generate_xml_with_signer(mock_xml_bytes, mock_sign, mock_load_signer):
    # Setup mock signer and cfdi return
    mock_signer = MagicMock()
    mock_load_signer.return_value = mock_signer
    mock_xml_bytes.return_value = b"<cfdi></cfdi>"
    ...
    assert xml == b"<cfdi></cfdi>"

@patch('notaria_4_core.backend.lib.xml_generator.load_signer_from_secret_manager')
def test_generate_xml_no_signer(mock_load_signer):
