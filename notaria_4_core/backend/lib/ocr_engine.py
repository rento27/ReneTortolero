...
def extract_structured_data(text: str) -> dict:
    ...
    # Fallback logic for vendedores and adquirientes if empty
    if not data["vendedores"]:
        vendedor_matches = re.findall(r"COMPARECE(?:N?)\s+([^,\n]+)", text, re.IGNORECASE)
        if vendedor_matches:
            data["vendedores"].extend([m.strip() for m in vendedor_matches])

    if not data["adquirientes"]:
        adquiriente_matches = re.findall(r"COMPRA(?:N?)\s+([^,\n]+)", text, re.IGNORECASE)
        if adquiriente_matches:
            data["adquirientes"].extend([m.strip() for m in adquiriente_matches])
