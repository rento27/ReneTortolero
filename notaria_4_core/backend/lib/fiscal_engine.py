...
def get_remote_config_sync():
    """Sync wrapper for remote config fetch."""
    try:
        # Initialize default app if not already
...
def validate_postal_code(cp: str, expected_state: str = None) -> bool:
    ...
    # Using a simple check if the document exists for the given postal code
...
def calculate_isai_manzanillo(operation_price: Decimal, cadastral_value: Decimal, rate: Decimal = None) -> Decimal:
    ...
    Default rate is fetched from Firebase Remote Config.
    """
