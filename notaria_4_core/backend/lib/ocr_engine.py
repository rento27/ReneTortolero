...
try:
    ...
    try:
        ...
        logger.warning(f"spaCy model 'ner_notaria' not found. Trying fallback. {e1}")
        try:
...
def extract_structured_data(text: str) -> dict:
    ...
    # Fallback logic if NLP fails to populate
    if not data["vendedores"]:
        # Fallback to looking for "COMPARECE"
        comparece_match = re.search(r"COMPARECE(?:N)?(?:\s*:)?\s+([A-Z\s,]+)(?:\s+Y|\s+CON|\n)", text, re.IGNORECASE)
        if comparece_match:
            data["vendedores"].append(comparece_match.group(1).strip())

    if not data["adquirientes"]:
        # Fallback to looking for "COMPRA"
        compra_match = re.search(r"COMPRA(?:N)?(?:\s*:)?\s+([A-Z\s,]+)(?:\s+Y|\s+CON|\n)", text, re.IGNORECASE)
        if compra_match:
